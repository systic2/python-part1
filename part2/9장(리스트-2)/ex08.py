...
if search_word in list_word:
    ...
    print("찾고자 하는 단어가", index, "번째에 존재합니다.")
else:
...
# 순차탐색은 리스트의 요소를 순서대로 하나씩 꺼내서 탐색키 값과 비교해서
# 성공하면 루프를 빠져나오고 루프를 다 했음에도 없다면 탐색키가 존재하지 않음을 의미한다.
...
if cnt == 0:    # cnt가 0이라면 찾고자 하는 값이 존재하지 않는다.
...
# 그 키 값 이상인 값들의 총 갯수를 구하는 프로그램을 작성해보시오.

# 사용자로부터 키를 입력
key_num = int(input("숫자를 입력하세요 : "))
result = []
for i in num_list:
    if i >= key_num:
        result.append(i)
print(f"입력한 키 값 : {result}, 키 값 이상인 값들의 총 갯수 : {len(result)}")
