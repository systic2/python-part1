# 전역변수(globla variable) : 함수의 외부에 정의된 변수를 전역변수라고 칭한다.
...
#
# # test()  # 함수 호출 시점에 전역변수 text 가 정의가 되어 있지 않기 때문에 에러가 발생
# text = "파이썬 공부"  # 전역변수인 text 변수에 문자열 할당
# test()

def test():
    # 지역변수 text 변수에 문자열 할당, 전역변수 text도 할당이 되어 있지만
    # 함수 내에서는 지역변수가 무조건 출력된다. 그 이유는 바로 함수 내에서는 지역변수가 디폴트이기 때문이다(중요)
    text = '파이썬 공부'
