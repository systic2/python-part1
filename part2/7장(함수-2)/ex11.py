...
# 파이썬에서는 튜플을 이용하여 여러 개의 값을 반환할 수가 있다.
# 튜플(tuple)은 몇 가지만 제외하고 리스트와 거의 비슷한 자료구조 형태이다.
# 리스트는 []으로 값을 둘러싸지만, 튜플은 ()로 값을 둘러싸는 형태이다.
# 리스트는 변경가능한 객체(생성, 삭제, 수정, 추가)이지만,
# 튜플은 한 번 값으로 만들면 그 값을 변경할 수 없는 객체이다.

# 파이썬에서는 튜플의 형태로 여러 개의 리턴을 하긴 하지만,
# 함수는 한 가지의 값을 리턴해준다는 그 정설에는 부합한다.
