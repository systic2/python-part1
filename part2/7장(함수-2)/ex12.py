...
# 파이썬에서는 무명함수를 만들 때 lambda 키워드를 만들 수 있다
...
def main():
    ...
    # 가장 많이 사용되는 곳은 GUI 프로그램에서 이벤트를 처리하는 콜백 함수 형태로 많이 사용된다.
    sum = lambda x, y: x + y

    print("두 정수의 합 : ", sum(10, 50))
    print("두 정수의 합 : ", sum(100, 50))

    # 람다함수를 출력을 하면 결과론적으로 함수 객체를 출력하는 형태가 된다.
