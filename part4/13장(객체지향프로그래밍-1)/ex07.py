...
# 인스턴스 변수는 메모리 공간에 독립적으로 공간을 차지하고 있어서 해당 인스턴스에게만 영향을 끼친다.
# 인스턴스 변수 접근 방법 : 인스턴스명.인스턴스 변수명
# 클래스 변수 : 클래스가 로딩이 되면서 메모리 상단(메소드 영역, 클래스 영역) 미리 공간을 할당하고 저장된다.
...
class Car:
    ...
    def __str__(self):
        ...
        # 객체지향 개념에서는 클래스명.클래스변수명으로 접근하는 것이 올바른 방법이다.
...
if __name__ == "__main__":
    ...
    car1 = Car()
    car1.__str__()
    # 파이썬에서는 인스턴스를 생성하면 메모리의 값이 새로이 할당됨을 알 수 있다.
    print(id(Car.count))
