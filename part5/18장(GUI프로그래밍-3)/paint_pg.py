...
def choose_color():
    global myColor
    # askcolor() 의 리턴타입은 튜플인데 인덱스[1]이 16진수로 색상을 표시
    myColor = askcolor(color=myColor)[1]
    print(f"변경된 색상 : {myColor}")
...
def paint(event):
    ...
    # 모드가 지우개이면 fill_color 를 white로 설정하고 아니면 myColor로 설정
...
canvas.bind("<ButtonRelease>", reset)
