# collections 모듈이란 기존에 배웠던 자료구조(리스트, 큐, 스택, 튜플, 딕셔너리) 좀 더 확장하여
# 제작된 파이썬 내장 모듈이다.
# 1) deque(데크, double-ended queue) 모듈은 스택과 큐를 모두 지원하는 모듈이다.
...
# print(deque_list.pop(0))  # deque 에서는 pop(0)이 지원이 안된다.
# deque 에서는 스택도 지원하는 모듈이므로 pop()을 사용하면 늦게 들어간 데이터부터 삭제됨을 알 수 있다.
...
print("--------------------------------")
# appendleft() 메소드 사용 :
# 새로운 요소들을 왼쪽부터 입력되게 하여 먼저 들어간 값부터 출력될 수 있도록 한다.
...
print(deque_list)
print("--------------------------------")
# deque 모듈은 원형 연결리스트(Linked List)의 특성을 지원을 한다.
...
print(deque_list)
# popleft() 메소드는 deque 에서 요소의 왼쪽에 있는 것을 삭제시킨다.
