...
# most_common(n) : 입력된 값의 요소들 중에서 빈도수가 높은 순으로 상위 n개를 리스트 안에 튜플 형태로 반환
count = Counter("apple, orange, grape")
# 매개변수를 주지 않았을 때는 전체 문자열을 대상으로 값과 빈도수를 출력
print(count.most_common())
# 매개변수를 주었을 때는 전체 문자열을 대상으로 빈도수를 순서가 높은 순으로 n개 만큼 출력
print(count.most_common(5))

print("---------------------------------")

# substract() : 단어 그대로 요소의 값을 빼는 것을 의미한다.
...
print(c2)
print(c1 + c2)
# 연산자로 뺄셈을 하면 음수의 값은 출력하지 않는다(substract와 차이점)
print(c1 - c2)
...
# 덧셈과 뺄셈만 Counter 모듈은 허용하지만 *, /, % 연산자는 허용하지 않는다
