...
import time  # 성능 테스트를 하기 위해서 time 모듈을 가지고 옴
# 저장 성능 테스트
# 아무런 요소가 없는 deque를 생성
deque_test = deque()
...
# 결론은 list, deque 를 성능 테스트를 해보니 데이터를 추가할 때는 deque 가 훨씬 좋은 성능을 발휘한다.
# 또한 대용량일수록 더욱 더 많은 차이를 나타낸다.
# 추출할 때는 역시 deque가 list보다 훨씬 빠른 성늘을 보인다.
# 데이터 용량이 크면 클수록 deque를 사용을 하는 것을 고려해 봐야 한다.
